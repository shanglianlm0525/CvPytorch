    ...
    def run(self):
        ...
        for epoch in range(self.start_epoch + 1, self.cfg.N_MAX_EPOCHS):
            ...
            if self.cfg.DATASET.VAL and (not (epoch+1) % cfg.EVALUATOR.EVAL_INTERVALS or epoch==self.cfg.N_MAX_EPOCHS-1):
                acc, perf_rst = self.val_epoch(epoch, self.ema.ema if cfg.EMA else model_ft, datasets['val'], dataloaders['val'])

                if cfg.local_rank == 0:
                    # start to save best performance model after learning rate decay to 1e-6
                    if best_acc < acc:
                        self.ckpts.autosave_checkpoint(deepcopy(self.ema.ema if cfg.EMA else model_ft), epoch, 'best', optimizer_ft)
                        best_acc = acc
            ...
            if not epoch % cfg.N_EPOCHS_TO_SAVE_MODEL:
                if cfg.local_rank == 0:
                    self.ckpts.autosave_checkpoint(deepcopy(self.ema.ema if cfg.EMA else model_ft), epoch,'last', optimizer_ft)
