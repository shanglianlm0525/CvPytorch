    ...
    def _parser_datasets(self):
        ...
        *dataset_str_parts, dataset_class_str = cfg.DATASET.CLASS.split(".")
        ...
        dataloaders = {x: DataLoader(datasets[x], batch_size=self.batch_size_all, num_workers=cfg.NUM_WORKERS,
                                     shuffle=cfg.DATASET[x.upper()].SHUFFLE, collate_fn=detection_collate,  pin_memory=True) for x in ['train', 'val']}
...
if __name__ == '__main__':
    ...
    parser.add_argument('--setting', default='conf/coco.yml', help='The path to the configuration file.')
