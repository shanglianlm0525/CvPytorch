...
from src.models.backbones.ghostnet import GhostBottleneck
...
class GhostPAN(nn.Module):
    ...
    Args:
        ...
        activation (str): Activation layer name.
            Default: LeakyReLU.
    """
    def __init__(self, in_channels, out_channels, kernel_size=3, num_extra_levels=1, use_depthwise=True, expand=1, num_blocks=1, use_res=False, norm_cfg=dict(type="BN"), activation='LeakyReLU'):
        # 'in_channels': [116, 232, 464], 'out_channels': 96, 'kernel_size': 5, 'num_extra_level': 1, 'use_depthwise': True, 'activation': 'LeakyReLU'
    ...
    def forward(self, x):
        ...
        inputs = [
            reduce(input_x) for input_x, reduce in zip(x, self.reduce_layers)
        ]
        # top-down path
        inner_outs = [inputs[-1]]
