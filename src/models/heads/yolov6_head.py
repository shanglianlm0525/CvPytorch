...
class YOLOv6Head(nn.Module):
    ...
    '''
    def __init__(self, subtype='yolov6_s', num_classes=80, in_channels=[128, 256, 512], num_anchors=1, num_layers=3, reg_max=16, depth_mul=1.0, width_mul=1.0):  # detection layer
        super().__init__()
        ...
        self.use_dfl = True if depth_mul > 0.5 else False # for m/l
        if self.use_dfl:
    ...
    def forward(self, x):
        ...
        return (None, x)  if self.training else (torch.cat(z, 1), None)