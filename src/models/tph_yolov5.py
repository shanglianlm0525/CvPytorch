...
from src.losses.tph_yolov5_loss import TPH_Yolov5Loss
...
class TPH_YOLOv5(nn.Module):
    ...
    def __init__(self, dictionary=None, model_cfg=None):
        super(TPH_YOLOv5, self).__init__()
        self.dictionary = dictionary
        self.model_cfg = model_cfg
        self.dummy_input = torch.zeros(1, 3, 1536, 1536)
        ...
        self.backbone_layer4 = nn.Sequential(Conv(in_places[3], in_places[4], 3, 2),    # 6
                                    C3TR(in_places[4], in_places[4], layers[3]),
        ...
        self.head_up_3_conv = Conv(in_places[2], in_places[1], 1, 1)                    # 18
        self.head_up_3 = nn.Sequential(SPP(in_places[1] * 2, in_places[1], [5, 9, 13]),
                                    C3(in_places[1], in_places[1], layers[6], False),
                                    CBAM(in_places[1], in_places[1]))                   # 23

        self.head_down_1_conv = Conv(in_places[1], in_places[1], 3, 2)
        self.head_down_1 = nn.Sequential(SPP(in_places[2] * 2, in_places[2], [5, 9, 13]),
                                    C3(in_places[2], in_places[2], layers[7], False),
                                    CBAM(in_places[2], in_places[2]))                   # 28

        self.head_down_2_conv = Conv(in_places[2], in_places[2], 3, 2)
        self.head_down_2 = nn.Sequential(SPP(in_places[3] * 2, in_places[3], [3, 7, 11]),
                                         C3(in_places[3], in_places[3], layers[8], False),
                                         CBAM(in_places[3], in_places[3]))              # 33

        self.head_down_3_conv = Conv(in_places[3], in_places[3], 3, 2)
        self.head_down_3 = nn.Sequential(SPP(in_places[4], in_places[4], [3, 5, 7]),
                                         C3TR(in_places[4], in_places[4], layers[9], False),
                                         CBAM(in_places[4], in_places[4]))              # 38

        self.detect = Detect(num_classes=self.num_classes, stride=stride, anchors=self.anchors, ch=in_places[1:])

        self.loss = TPH_Yolov5Loss(self.num_classes, self.anchors)
    ...
    def forward(self, imgs, targets=None, mode='infer', **kwargs):
        ...
        if mode == 'infer':
            ...
            out18 = self.head_up_3_conv(out17)
            out23 = self.head_up_3(torch.cat([F.interpolate(out18, scale_factor=2, mode="nearest"), out2], dim=1))

            out28 = self.head_down_1(torch.cat([self.head_down_1_conv(out23), out18, out4], dim=1))
            out33 = self.head_down_2(torch.cat([self.head_down_2_conv(out28), out14, out6], dim=1))
            out38 = self.head_down_3(torch.cat([self.head_down_3_conv(out33), out10], dim=1))

            out, train_out = self.detect([out23, out28, out33, out38])
