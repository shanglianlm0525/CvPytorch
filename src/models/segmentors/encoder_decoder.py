...
class EncoderDecoder(BaseSegmentor):
    ...
    def setup_extra_params(self):
        ...
        if self.model_cfg.LOSS is not None:
            if isinstance(self.model_cfg.LOSS, list):
                for head_cfg in self.model_cfg.LOSS:
                    head_cfg.__setitem__('num_classes', self.num_classes)
            else:
                self.model_cfg.LOSS.__setitem__('num_classes', self.num_classes)
        '''

    def loss_forward(self, preds, targets):
        ...
        if self.with_auxiliary_head:
            ...
            if isinstance(self.auxiliary_head, nn.ModuleList):
                for aux_head in self.auxiliary_head:
                    preds.append(aux_head(feats))
            else:
                preds.append(self.auxiliary_head(feats))

        if mode == 'infer':

            return torch.argmax(feats, dim=1)
        else:
            losses = self.loss_forward(preds, targets)
