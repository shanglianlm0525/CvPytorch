...
# @Author : liumin
# @File : tph_yolov5_backbone.py

import torch
import torch.nn as nn
from src.models.modules.yolov5_modules import Conv, C3, C3TR, SPPF


class TPH_YOLOv5Backbone(nn.Module):
    def __init__(self, subtype='tph_yolov5s', out_stages=[2,3,4], output_stride=32, layers=[3, 6, 9, 3], depth_mul=1.0, width_mul=1.0, backbone_path=None, pretrained = False):
        super(TPH_YOLOv5Backbone, self).__init__()
        ...
        self.layer4 = nn.Sequential(Conv(in_places[3], in_places[4], 3, 2),
                                    ...
                                    Conv(in_places[4], in_places[3], 1, 1))
...
if __name__ == "__main__":
    model = TPH_YOLOv5Backbone(subtype='yolov5s', depth_mul = 0.33, width_mul = 0.50)
    print(model)
