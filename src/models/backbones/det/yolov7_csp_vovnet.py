# !/usr/bin/env python
# -- coding: utf-8 --
# @Time : 2022/10/28 17:47
# @Author : liumin
# @File : yolov7_csp_vovnet.py
...
from src.models.modules.yolo_modules import EELAN
...
class YOLOv7CSPVoVNet(BaseYOLOBackbone):
    ...
    def __init__(self, subtype='cspdark_s', in_channels=3, out_channels=[64, 128, 256, 512, 1024], num_blocks=[6, 12, 18, 6], spp_ksizes=5, depthwise=False,
        ...
        super(YOLOv7CSPVoVNet, self).__init__(subtype=subtype, cfg=self.cfg, in_channels=in_channels, out_channels=out_channels, num_blocks=num_blocks, depthwise=depthwise,
    ...
    def build_stem_layer(self):
        """Build a stem layer."""
        return nn.Sequential(ConvModule(self.in_channels, self.out_channels[0], kernel_size=3, stride=1, padding=1,
                                        norm_cfg=self.norm_cfg, act_cfg=self.act_cfg),
                             ConvModule(self.out_channels[0], self.out_channels[1], kernel_size=3, stride=2, padding=1,
                                        norm_cfg=self.norm_cfg, act_cfg=self.act_cfg),
                             ConvModule(self.out_channels[1], self.out_channels[1], kernel_size=3, stride=1, padding=1,
                                        norm_cfg=self.norm_cfg, act_cfg=self.act_cfg)
                             )
    ...
    def train(self, mode=True):
        super(YOLOv7CSPVoVNet, self).train(mode)
        self._freeze_stages()
...
if __name__ == "__main__":
    model = YOLOv7CSPVoVNet('cspdark_m')
    print(model)
