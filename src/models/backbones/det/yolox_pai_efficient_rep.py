# !/usr/bin/env python
# -- coding: utf-8 --
# @Time : 2022/10/28 17:51
# @Author : liumin
# @File : yolox_pai_efficient_rep.py
...
from src.models.modules.yolo_modules import SPPF, RepVGGBlock


class YOLOXPAIEfficientRep(BaseYOLOBackbone):
    cfg = {"n": [0.33, 0.25],
            "t": [0.33, 0.375],
            "s": [0.33, 0.5],
            "m": [0.67, 0.75],
            "l": [1.0, 1.0],
            "x": [1.33, 1.25]}
    def __init__(self, subtype='yolox_pai_s', in_channels=3, out_channels=[64, 128, 256, 512, 1024], num_blocks=[6, 12, 18, 6], spp_ksizes=5, depthwise=False,
        ...
        super(YOLOXPAIEfficientRep, self).__init__(subtype=subtype, cfg=self.cfg, in_channels=in_channels, out_channels=out_channels, num_blocks=num_blocks, depthwise=depthwise,
    ...
    def build_stage_layer(self, idx, in_channels, out_channels, num_blocks):
        ...
        for i in range(num_blocks):
            stage.append(RepVGGBlock(out_channels, out_channels))
        if idx == 3:
    ...
    def train(self, mode=True):
        super(YOLOXPAIEfficientRep, self).train(mode)
        self._freeze_stages()
...
if __name__ == "__main__":
    model = YOLOXPAIEfficientRep('yolox_pai_s')
    print(model)
