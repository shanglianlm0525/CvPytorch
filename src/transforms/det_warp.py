...
def warp_and_resize(sample, warp_kwargs, dst_shape, keep_ratio=True):
    # TODO: background, type
    img, target = sample['image'], sample['target']

    raw_img = np.asarray(img, dtype=np.float32)
    # raw_img = meta['img']
    ...
    if 'masks' in target and False:
        for i, mask in enumerate(target['masks']):
            target['masks'][i] = cv2.warpPerspective(mask, M, dsize=tuple(dst_shape))

    target['warp_matrix'] = torch.as_tensor(target['warp_matrix'], dtype=torch.float32)
