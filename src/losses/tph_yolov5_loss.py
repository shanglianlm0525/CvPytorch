# !/usr/bin/env python
# -- coding: utf-8 --
# @Time : 2021/11/23 8:42
# @Author : liumin
# @File : tph_yolov5_loss.py
import math

import torch
import torch.nn as nn

from src.utils.ema import is_parallel
...
class TPH_Yolov5Loss:
    # Compute losses
    def __init__(self, num_classes, anchors, num_layers=4, num_anchors=4, stride=[4.,  8., 16., 32.], device='cuda:0', autobalance=False):
        self.sort_obj_iou = False
        self.num_classes = num_classes
        self.num_layers = num_layers  # number of detection layers
        self.num_anchors = num_anchors  # number of anchors
        self.device = device
        self.anchors = torch.tensor(anchors, device=self.device).view(len(anchors), 4, -1)
        self.stride = torch.tensor(stride, device=self.device).view(-1, 1, 1)

        self.hyp_anchor_t = 3.0
        self.hyp_box = 0.0525 #  3 * 0.07 / self.num_layers
        self.hyp_cls = 0.016875 # 0.18 * self.num_classes / 80 * 3 / self.num_layers
        self.hyp_obj = 0.648 # 0.15 * (1536 / 640) ** 2 * 3 / self.num_layers

        self.hyp_cls_pw = 0.631
        self.hyp_obj_pw = 0.911
        self.hyp_label_smoothing = 0.0
        self.hyp_fl_gamma = 0.0
        ...
        self.balance = {3: [4.0, 1.0, 0.4]}.get(self.num_layers, [4.0, 1.0, 0.25, 0.06, 0.02])  # P3-P7
