# !/usr/bin/env python
# -- coding: utf-8 --
# @Time : 2021/8/23 16:28
# @Author : liumin
# @File : yolofastestv2_loss.py
import math
...
class YoloFastestv2Loss(nn.Module):
    ...
    def forward(self, preds, targets):
        ...
        # 构建gt
        tcls, tbox, indices, anchors = self.build_target(preds, targets)
        for i, pred in enumerate(preds):
            # 计算reg分支loss
            if i % 3 == 0:
                pred = pred.reshape(pred.shape[0], self.num_anchors, -1, pred.shape[2], pred.shape[3])
            ...
            # 计算obj分支loss
                ...
                lobj += self.BCEobj(pred[..., 0], tobj) * self.balance[layer_index[i]]  # obj loss
                ...
                if len(indices):
                    ...
                    if nb:
                        ...
                        if ps.size(1) > 1:
                            ...
                            lcls += self.BCEcls(ps[:, :], tcls[layer_index[i]]) / self.num_classes  # BCE
            else:
                print("error")
                raise
    ...
    def build_target(self, preds, targets):
        ...
        anchor_num, label_num = self.num_anchors, targets.shape[0]
        # 加载anchor配置
        anchors = torch.from_numpy(np.array(self.anchors).reshape(len(preds) // 3, anchor_num, 2)).to(device)
