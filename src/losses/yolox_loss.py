...
class YoloxLoss(nn.Module):
    def __init__(self, num_classes, use_l1=False):
        super().__init__()
    ...
    def get_assignments(
        ...
        cls_preds_ = (cls_preds_.float().unsqueeze(0).repeat(num_gt, 1, 1).sigmoid_()
                      * obj_preds_.unsqueeze(0).repeat(num_gt, 1, 1).sigmoid_())
        pair_wise_cls_loss = F.binary_cross_entropy_with_logits(cls_preds_.sqrt_(), gt_cls_per_image, reduction="none").sum(-1)
        del cls_preds_
